...
class MetaRLAlgorithm(metaclass=abc.ABCMeta):
    ...
    def train(self):
        ...
        for it_ in gt.timed_for(
            ...
            for i in range(self.num_tasks_sample):  # 5次
                idx = np.random.randint(len(self.train_tasks))
                self.task_idx = idx
                self.env.reset_task(idx)
    ...
    def get_extra_data_to_save(self, epoch):
        ...
        if self.save_replay_buffer:
            data_to_save['replay_buffer'] = self.replay_buffer.task_buffers
        if self.save_algorithm:
            data_to_save['algorithm'] = self
        return data_to_save
    
    def get_extra_data_to_save_mmd(self, epoch, task):
        """
        Save things that shouldn't be saved every snapshot but rather
        overwritten every time.
        :param epoch:
        ...
        if self.save_environment:
            data_to_save['env'] = self.training_env
        if self.save_replay_buffer:
            data_to_save['replay_buffer'] = self.replay_buffer.task_buffers[task]
        if self.save_algorithm:
            data_to_save['algorithm'] = self
        return data_to_save
    ...
    def collect_paths(self, idx, epoch, run):
        ...
        while num_transitions < self.num_steps_per_eval:
            ...
            if num_trajs >= self.num_exp_traj_eval:
                ...
                # print(self.agent.context.shape)
    ...
    def evaluate_meta_test(self, indices, num_epoch):
        """
        meta test 阶段的测试。与indices中的任务分别交互 iter 次，每次60step，
        :param indices:
        :return:
        """
        if self.eval_statistics is None:
        ...
        for epoch in range(num_epoch):
            ...
            avg_test_return = np.mean(test_final_returns)
            avg_test_online_return = np.mean(np.stack(test_online_returns), axis=0)
            self.eval_statistics['Eval_task_num'] = indices[0]  
            self.eval_statistics['AverageReturn_all_test_tasks'] = avg_test_return
            # logger.save_extra_data(avg_test_online_return, path='online-test-epoch{}'.format(epoch))
            for key, value in self.eval_statistics.items():
                logger.record_tabular(key, value)
